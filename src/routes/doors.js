...
router.post('/login', auth, async (req, res) => {
  ...
  try {
    ...
    if (!door) {
      return res.status(401).json({ error: 'Invalid door name or password' });
    }
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO door_access (door_id, user_id, approved_by)
       VALUES ($1, $2, $3)
       ON CONFLICT (door_id, user_id) DO NOTHING`,
...
router.delete('/:doorId/users/:userId', auth, async (req, res) => {
  ...
  try {
    ...
    if (!owner) {
      return res.status(403).json({ error: 'Only the door owner can remove users' });
    }
...
router.post('/open', auth, async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE doors SET is_open = TRUE WHERE id = $1', [door_id]);
    await pool.query(
...
router.post('/close', auth, async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE doors SET is_open = FALSE WHERE id = $1', [door_id]);
    await pool.query(
      'INSERT INTO door_logs (user_id, door_id, action) VALUES ($1, $2, $3)',
...
router.delete('/:doorId', auth, async (req, res) => {
  ...
  try {
    ...
    if (!owner) {
      return res.status(403).json({ error: 'Only the door owner can delete this door' });
    }
